...
@Component({
  ...
  template: `
    ...
    >
      <header
        class="flex justify-between items-end mb-8 border-b border-border/40 pb-4"
      >
        <div>
          <nav
            class="flex text-[0.75rem] font-[800] uppercase tracking-wider text-muted-foreground mb-4"
          >
            <span>Sistema</span>
          ...
          <div class="flex items-center gap-3">
            ...
            <h1
              class="text-[clamp(1.5rem,2vw,2rem)] font-display font-medium tracking-tight m-0 text-foreground"
            >
              Centro de Insights AI
            </h1>
          </div>
          <p class="text-sm text-muted-foreground mt-2 max-w-2xl">
            Sugerencias, patrones y conclusiones de negocio abstraídas de forma
            autónoma por los asistentes de Inteligencia Artificial del ERP.
          </p>
        </div>
      </header>
      ...
      <div
        *ngIf="loading()"
        class="py-12 flex justify-center text-muted-foreground"
      >
        Cargando insights...
      </div>

      <div
        *ngIf="!loading() && filteredInsights().length === 0"
        class="py-24 text-center border border-dashed border-border/60 rounded-2xl bg-card/20"
      >
        <lucide-icon
          name="brain-circuit"
          class="w-12 h-12 text-muted-foreground/50 mx-auto mb-4"
        ></lucide-icon>
        <h3 class="text-lg font-medium text-foreground mb-2">
          No hay Insights registrados
        </h3>
        <p class="text-muted-foreground max-w-md mx-auto text-sm">
          Aún no hay sugerencias de la IA. Pídele al Orquestador o a los Bots
          Especialistas que analicen tus datos y guarden descubrimientos.
        </p>
      </div>
        ...
        >
          <div class="flex justify-between items-start mb-4">
            ...
            <div class="flex gap-2 items-center text-xs text-muted-foreground">
              <span class="px-2 py-1 bg-secondary rounded-lg">{{
                insight.feature
              }}</span>
            </div>
          </div>
          ...
          <h3
            class="text-lg font-medium text-foreground mb-3 leading-snug group-hover:text-primary transition-colors"
          >
            {{ insight.title }}
          </h3>
          ...
          <div
            *ngIf="insight.metrics && hasKeys(insight.metrics)"
            class="mt-auto mb-6 p-4 bg-secondary/50 rounded-xl space-y-2"
          >
            <div
              *ngFor="let key of getKeys(insight.metrics)"
          ...
          <div
            class="mt-auto pt-4 border-t border-border/40 flex items-center justify-between text-xs text-muted-foreground"
          >
...
export class AiInsightsComponent implements OnInit {
  ...
  filteredInsights = computed(() => {
    ...
    if (!term) return this.insights();
    return this.insights().filter(
      (insight) =>
        insight.title.toLowerCase().includes(term) ||
        insight.summary.toLowerCase().includes(term) ||
        insight.feature.toLowerCase().includes(term) ||
        insight.botId.toLowerCase().includes(term),
    );
  });
  ...
  async loadInsights() {
    ...
    try {
      const data = await firstValueFrom(
        this.http.get<AiInsight[]>('/api/ai-insights'),
      );
