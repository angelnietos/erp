  ...
  template: `
    <div class="space-y-4">
      <div class="flex items-center gap-2">
        ...
        />
        <label for="watermarkEnabled" class="text-sm font-medium text-slate-700"
          >Activar marca de agua</label
        >
      </div>
          ...
          <div class="space-y-3">
            <div>
              ...
              <input
                ...
                class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
              />
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div>
                <label
                  for="watermarkFontSize"
                  class="block text-xs font-medium text-slate-600 mb-1"
                  >Tamaño fuente (px)</label
                >
                <input
                  type="number"
            ...
            @if (config.text) {
              <div class="mt-2 p-2 bg-white rounded border border-slate-200">
                ...
                <div
                  class="relative h-16 overflow-hidden rounded bg-slate-50/50 flex items-center justify-center"
                >
                  <span
                    style="font-size: {{ config.fontSize }}px; color: {{
                      config.color
                    }}; opacity: {{ config.opacity }}; transform: rotate({{
                      config.rotation
                    }}deg); font-weight: 700;"
                  >
                    {{ config.text }}
