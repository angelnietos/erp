...
export const PDF_COVER_A4_STYLE =
  'width: 210mm; height: 297mm; min-height: 297mm; box-sizing: border-box; page-break-after: always; break-after: page;';
...
export function exportCoverConfigToHtml(c: Partial<CoverConfig>): string {
  ...
  const subtitleFontSize =
    c.htmlSubtitleFontSize ?? c.subtitleFontSize ?? '1.1rem';
  const metadataItems = [
...
export function exportSignatureConfigToHtml(
...
function renderSignatureBlock(c: Partial<SignatureConfig>): string {
  ...
  if (c.showLine) {
    blocks.push(
      '<div style="border-top: 1px solid #374151; margin: 0 auto 8px; min-width: 180px;"></div>',
    );
  }
  blocks.push(
  ...
  if (c.title) {
    blocks.push(
      `<div style="color: #6b7280; font-size: 0.8rem;">${c.title}</div>`,
    );
  }
  if (c.company) {
    blocks.push(
      `<div style="color: #6b7280; font-size: 0.8rem;">${c.company}</div>`,
    );
  }
  ...
  if (locationAndDate) {
    blocks.push(
      `<div style="color: #9ca3af; font-size: 0.75rem; margin-top: 8px;">${locationAndDate}</div>`,
    );
  }
...
export function exportHeaderFooterConfigToHtml(
  ...
  const samplePage = Number.isFinite(c.startPageFrom ?? NaN)
    ? c.startPageFrom!
    : 1;
...
export function exportWatermarkConfigToHtml(
  c: Partial<WatermarkConfig>,
): string {
...
export function assembleDocumentBodyHtml(
  ...
  const coverEnabled = extras.coverPanelEnabled || extras.coverConfig?.enabled;
