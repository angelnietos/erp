...
export function scopeSingleSelector(selector: string): string {
  ...
  if (
    selector.includes('document-create-shell') &&
    selector.includes('document-preview-pane')
  ) {
    return selector;
  }
...
export function prioritizeUserCss(css: string): string {
  ...
  return prioritized.replace(
    ...
    (match, boundary: string, selectorText: string, body: string) => {
      const colorDeclaration = /\bcolor\s*:\s*[^;!}]+(?:\s*!important)?/i.test(
        body,
      );
...
export function enrichDocumentHtmlForStyling(html: string): string {
  ...
  try {
    ...
    root
      .querySelectorAll('p')
      .forEach((paragraph) => addClasses(paragraph, 'doc-paragraph'));
    root
      .querySelectorAll('ul')
      .forEach((list) => addClasses(list, 'doc-list', 'doc-list--unordered'));
    root
      .querySelectorAll('ol')
      .forEach((list) => addClasses(list, 'doc-list', 'doc-list--ordered'));
    root
      .querySelectorAll('li')
      .forEach((item) => addClasses(item, 'doc-list-item'));
    root
      .querySelectorAll('blockquote')
      .forEach((quote) => addClasses(quote, 'doc-callout'));
    root.querySelectorAll('table').forEach((table, index) => {
      addClasses(table, 'doc-table', `doc-table--${index + 1}`);
    });
    root
      .querySelectorAll('thead')
      .forEach((thead) => addClasses(thead, 'doc-table-head'));
    root
      .querySelectorAll('tbody')
      .forEach((tbody) => addClasses(tbody, 'doc-table-body'));
    root
      .querySelectorAll('tr')
      .forEach((row) => addClasses(row, 'doc-table-row'));
    root
      .querySelectorAll('th')
      .forEach((cell) => addClasses(cell, 'doc-table-header'));
    root
      .querySelectorAll('td')
      .forEach((cell) => addClasses(cell, 'doc-table-cell'));
    root
      .querySelectorAll('hr')
      .forEach((divider) => addClasses(divider, 'doc-divider'));
    root
      .querySelectorAll('pre')
      .forEach((code) => addClasses(code, 'doc-code-block'));
    root
      .querySelectorAll('code')
      .forEach((code) => addClasses(code, 'doc-code'));
    root
      .querySelectorAll('img')
      .forEach((image) => addClasses(image, 'doc-image'));
 ...
 */
export function resolveStoredDocumentCss(
...
:where(.document-preview-pane--isolated.markdown-preview) th:not([style*='color:']),
:where(.document-preview-render.markdown-preview) th:not([style*='color:']),
:where(.pdf-body-content.markdown-preview) th:not([style*='color:']) {
 ...
 */
export function resolvePdfCustomCss(
  storedCustomCss: string | undefined,
): string {
...
.markdown-preview th:not([style*='color:']),
.document-preview-render.markdown-preview th:not([style*='color:']) {
   background: ${tableHeaderBg};
   color: ${text};
   border-color: ${border};
 }
