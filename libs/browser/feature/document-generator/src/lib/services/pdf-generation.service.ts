...
export class PdfGenerationService {
  ...
  private async htmlToPdfBlob(
    ...
    try {
      ...
      return await html2pdf()
        .set(options)
        .from(page)
        .toPdf(function (pdf: JsPdfInstance) {
          const totalPages = pdf.internal.getTotalPages
            ? pdf.internal.getTotalPages()
            : 1;
          ...
          for (let i = 1; i <= totalPages; i++) {
            ...
            pdf.text(`Página ${i}`, pageWidth / 2, pageHeight - 15, {
              align: 'center',
            });
          }
        })
  ...
  private expandPdfCanvasToFullPages(
    ...
    const canvasRoot = page.querySelector(
      '.pdf-canvas-root',
    ) as HTMLElement | null;
    if (canvasRoot) {
  ...
  async generateMarkdownPdf(data: DocumentData): Promise<Blob> {
    ...
    if (dataCopy.pdfBackgroundImageUrl) {
      dataCopy.pdfBackgroundImageUrl = await this.imageUrlToBase64(
        dataCopy.pdfBackgroundImageUrl,
      );
    }
    if (dataCopy.coverConfig) {
      ...
      if (typeof cover['backgroundImageUrl'] === 'string') {
        cover['backgroundImageUrl'] = await this.imageUrlToBase64(
    ...
    if (dataCopy.signatureConfig) {
      ...
      if (typeof sig['signatureImageUrl'] === 'string') {
        sig['signatureImageUrl'] = await this.imageUrlToBase64(
  ...
  private prepareHtmlContentForPdf(
    content: string,
    data: DocumentData,
  ): string {
  ...
  private buildHeaderFooterCss(data: DocumentData): string {
    const hf = (data as Record<string, unknown>)['headerFooterConfig'] as
      | Record<string, unknown>
      | undefined;
    if (!hf || hf['enabled'] !== true) return '';

    const fontSize =
      typeof hf['fontSize'] === 'string' ? hf['fontSize'] : '9pt';
    const textColor =
      typeof hf['textColor'] === 'string' ? hf['textColor'] : '#64748b';
    const showPageNumbers = hf['showPageNumbers'] !== false;
    const showDivider = hf['showDivider'] !== false;

    const dividerColor = textColor;
    const footerBackground =
      hf['backgroundColor'] && hf['backgroundColor'] !== 'transparent'
        ? hf['backgroundColor']
  ...
  private buildPdfHeaderFooterHtml(data: DocumentData): string {
    ...
    const showPageNumbers = hf['showPageNumbers'] !== false;
    const format =
      typeof hf['pageNumberFormat'] === 'string'
        ? hf['pageNumberFormat']
        : 'simple';
...
`,
  ...
  private buildCoverHtml(data: DocumentData): string {
    const cover = (data as Record<string, unknown>)['coverConfig'] as
      | Record<string, unknown>
      | undefined;
    ...
    const subtitle = escapeHtml(
      typeof cover['subtitle'] === 'string' ? cover['subtitle'] : '',
    );
    const author = escapeHtml(
      typeof cover['author'] === 'string' ? cover['author'] : '',
    );
    const date = escapeHtml(
      typeof cover['date'] === 'string'
        ? cover['date']
        : this.formatDisplayDate(data.date),
    );
    const logoUrl =
      typeof cover['logoUrl'] === 'string' ? cover['logoUrl'] : '';
    const textColor =
      typeof cover['textColor'] === 'string' ? cover['textColor'] : '#ffffff';
    const showDivider = cover['showDivider'] !== false;
    const showDate = cover['showDate'] !== false;
    const showAuthor = cover['showAuthor'] !== false;
    const layout =
      typeof cover['layout'] === 'string' ? cover['layout'] : 'centered';
    const titleFontSize =
      typeof cover['htmlTitleFontSize'] === 'string'
        ? cover['htmlTitleFontSize']
        : '2.5rem';
    const subtitleFontSize =
      typeof cover['htmlSubtitleFontSize'] === 'string'
        ? cover['htmlSubtitleFontSize']
        : '1.1rem';

    let backgroundStyle =
      'background: linear-gradient(135deg, #420000 0%, #7a0000 100%);';
    const bgType =
      typeof cover['backgroundType'] === 'string'
        ? cover['backgroundType']
        : 'gradient';
    if (bgType === 'solid' && typeof cover['backgroundColor'] === 'string') {
      backgroundStyle = `background: ${cover['backgroundColor']};`;
    } else if (
      bgType === 'gradient' &&
      typeof cover['gradientFrom'] === 'string' &&
      typeof cover['gradientTo'] === 'string'
    ) {
      backgroundStyle = `background: linear-gradient(135deg, ${cover['gradientFrom']}, ${cover['gradientTo']});`;
    } else if (
      bgType === 'image' &&
      typeof cover['backgroundImageUrl'] === 'string'
    ) {
      const imageUrl = cover['backgroundImageUrl']
        .replace(/"/g, '%22')
        .replace(/'/g, '%27');
    ...
    const metadataParts = [
      showAuthor && author
        ? `<span class="cover-meta-item">Autor: ${author}</span>`
        : '',
      showDate && date
        ? `<span class="cover-meta-item">Fecha: ${date}</span>`
        : '',
    ]
      .filter(Boolean)
      .join(showAuthor && showDate ? ' · ' : '');
  ...
  private buildWatermarkHtml(data: DocumentData): string {
    const wm = (data as Record<string, unknown>)['watermarkConfig'] as
      | Record<string, unknown>
      | undefined;
    ...
    const opacity = Math.max(
      0.05,
      Math.min(0.5, typeof wm['opacity'] === 'number' ? wm['opacity'] : 0.1),
    );
...
">${text}</div>`;
  ...
  private applyCorporateCoverVisibility(
    ...
    wrapper
      ...
      .forEach((coverEl) => coverEl.remove());
  ...
  private async generatePdfFromHtml(
    ...
    const metaClient = escapeHtml(data.subtitle || data.client || 'Josanz ERP');
