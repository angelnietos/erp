...
@Component({
  ...
  template: `
    <div class="space-y-8">
      ...
      >
        <a routerLink="/documents/list" class="hover:text-primary"
          >Documentos</a
        >
      ...
      <div class="bg-surface rounded-2xl shadow-xl border border-soft p-8">
        ...
        <p class="text-secondary max-w-3xl">
          Las <strong class="text-primary">skills genéricas</strong> alinean el
          tono y el formato para todos los usuarios. Las
          <strong class="text-primary">skills personales</strong> y la
          <strong class="text-primary">memoria</strong> se guardan en este
          navegador (IndexedDB
          <code
            class="text-xs bg-secondary border border-soft px-1.5 py-0.5 rounded text-primary"
            >josanz-document-agent</code
          >). Para memoria compartida entre equipos, puedes levantar Postgres
          con el compose descrito en
          <code class="text-xs text-secondary">deploy/document-agent/</code>.
        </p>
      </div>
        ...
        <div class="flex flex-wrap gap-3 items-end">
          <label class="flex flex-col gap-1">
            <span class="text-sm text-secondary"
              >Cómo debe recordarte el agente</span
            >
            <input
              type="text"
              [(ngModel)]="displayName"
      ...
      <div class="bg-surface rounded-2xl shadow border border-soft p-6">
        ...
        <p class="text-sm text-secondary mb-4">
          Activas para todos; puedes
          <strong class="text-primary">desactivar</strong> las que
          <span class="text-brand font-medium">no</span> apliquen a tu sesión.
        </p>
        <ul class="space-y-3">
      ...
      <div class="bg-surface rounded-2xl shadow border border-soft p-6">
        ...
        @if (userSkills.length === 0 && !skillDraft) {
          <p class="text-sm text-secondary">
            Aún no has añadido instrucciones propias.
          </p>
        }
          ...
          <div class="skill-card">
            @if (skillDraft && skillDraft.id === u.id) {
              ...
              <div class="flex gap-2 flex-wrap items-center">
                <label
                  class="inline-flex items-center gap-2 text-sm text-primary cursor-pointer"
                >
                  <input
                    type="checkbox"
        ...
        @if (skillDraft && skillDraft.id === undefined) {
          <div
            class="p-4 rounded-xl border border-dashed border-soft bg-secondary space-y-2"
          >
            <input
              [(ngModel)]="skillDraft.title"
      ...
      <div class="bg-surface rounded-2xl shadow border border-soft p-6">
        ...
        <p class="text-sm text-secondary mb-4">
          Notas que el agente incluirá en el system prompt (últimas entradas).
          Útil para preferencias estables ("siempre firmar como…", sectores,
          etc.).
        </p>
        ...
        <ul class="space-y-2">
          @for (m of memories; track m.id) {
            <li class="memory-item">
              <span class="whitespace-pre-wrap flex-1 text-primary">{{
                m.text
              }}</span>
              <button
                type="button"
