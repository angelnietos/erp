  ...
  template: `
    <div class="hf-editor-panel">
      <div class="flex items-center justify-between mb-4">
        <div>
          <h3 class="text-sm font-semibold text-blue-900">Encabezado y Pie de Página</h3>
          <p class="text-xs text-blue-700 mt-0.5">Elementos que aparecerán en cada página del PDF</p>
        </div>
        ...
        <div class="space-y-4">
          <div class="flex flex-wrap gap-2 mb-3">
            ...
            @for (v of variables; track v.code) {
              <span class="variable-chip" (click)="insertVariable(v.code)" [title]="v.label">
                {{ v.code }}
              </span>
            }
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="form-group">
              ...
              <select [ngModel]="config().fontSize" (ngModelChange)="update({ fontSize: $event })">
          ...
          <div>
            <label class="text-xs font-medium text-blue-700 mb-2 block">Vista previa</label>
            <div class="hf-preview-page">
              ...
              >
                <span>{{ resolveVariables(config().headerLeft) || 'Encabezado izq.' }}</span>
                <span>{{ resolveVariables(config().headerCenter) || 'Centro' }}</span>
                <span>{{ resolveVariables(config().headerRight) || 'Encabezado der.' }}</span>
              </div>
              <div class="hf-preview-body">
              ...
              <div
                ...
                [style.color]="config().textColor"
              >
                <span>{{ resolveVariables(config().footerLeft) || 'Pie izq.' }}</span>
                <span>{{ resolveVariables(config().footerCenter) || 'Centro' }}</span>
                <span>{{ resolveVariables(config().footerRight) || pageNumberPreview }}</span>
              </div>
            </div>
          </div>
        </div>
  ...
  insertVariable(variableCode: string): void {
    ...
    if (currentActive) {
      this.config.update((c) => ({
        ...c,
        [currentActive]: c[currentActive as keyof HeaderFooterConfig] + variableCode,
      }));
    }
  }
