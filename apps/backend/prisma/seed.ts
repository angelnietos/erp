...
const FULL_ACCESS_ROLE_PERMISSIONS = uniquePermissions([
  '*',
  ...ALL_APP_PERMISSIONS,
]);
...
async function ensureDefaultRoles(tenantId: string, tenantSlug: string) {
  ...
  if (!superAdminRole) {
    superAdminRole = await prisma.role.create({
      data: {
        ...
        description:
          'Super administrador: acceso total y configuración de roles y permisos',
      },
    });
  } else {
  ...
  if (isAdminTenant) {
    ...
    let ba = await prisma.role.findFirst({
      where: { tenantId, name: 'Admin Babooni' },
    });
    if (!ba) {
  ...
  let respRole = await prisma.role.findFirst({
...
// 2. Main Demo Tenant (Josanz) - Uses fixed UUID to match Keycloak realm config
  ...
  await prisma.userRole.upsert({
    where: {
      userId_roleId: {
        userId: babooniAdmin.id,
        roleId: babooniSuperAdminRole.id,
      },
    },
    update: {},
  ...
  await (prisma as any).eventReport.create({
    data: {
      ...
      content:
        'El montaje se completó según lo previsto. Todos los equipos de audio y video están operativos. Se recomienda personal adicional para el desmontaje.',
      authorId: admin.id,
    },
  });
  ...
  await (prisma as any).eventReport.create({
    data: {
      ...
      content:
        'Revisión final completada. Iluminación y plató listos para rodaje cerrado. Queda pendiente la calibración final del proyector láser en la escena nocturna. Cliente (Carlos) firmó el checklist.',
      authorId: admin.id,
    },
  });
...
async function seedBabooniTenantDemo(tenantId: string) {
  ...
  for (const row of technicianSeeds) {
    ...
    if (!u) {
      console.warn(
        `- Babooni seed: usuario no encontrado para técnico ${row.email}`,
      );
      continue;
    }
  ...
  console.log(
    '- Babooni: técnicos / operarios AV (usuarios semilla vinculados)',
  );
  ...
  const productDefs: {
    ...
    {
      name: 'Micrófono inalámbrico doble',
      sku: 'BB-MIC-01',
      stock: 12,
      price: 450,
      dailyRate: 15,
